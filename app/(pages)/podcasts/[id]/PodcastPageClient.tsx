...
import { formatDate, getTimeAgo } from '../../articles/[id]/ArticlePageClient'

// Utility function to convert YouTube URLs to embed format
const convertToEmbedUrl = (url: string): string => {
    if (!url) return url;

    // Regular YouTube URL pattern: https://www.youtube.com/watch?v=VIDEO_ID
    const watchRegex = /(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)/;
...
export default function PodcastPageClient({ id }: PodcastPageClientProps) {
    ...
    return (
        <div className="container mx-auto max-w-5xl mt-12 mb-12 px-4">
            ...
            <div className="mb-8">
                {hasYouTube ? (
                    <div className="relative w-full aspect-video rounded overflow-hidden shadow-lg mb-6">
                        <iframe
                            src={convertToEmbedUrl(podcast.youtubeLink)}
                            title={podcast.title}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    ...
                    {hasApple && (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center justify-center flex-1 bg-gray-100 dark:bg-white text-black py-3 px-4 rounded-lg hover:scale-102 transition-all font-medium"
                        >
                            <svg className="w-7 h-7 mr-2" fill="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex flex-wrap mb-5 gap-2">
                    ...
                    <div className="flex items-center gap-2 bg-[#F6BCB2] rounded-full text-black px-4 py-1.5">
                        ...
                        <span className="text-sm">{getTimeAgo(podcast.createdAt)}</span>
                    </div>

                    {/* Published Date */}
                    <div className="flex items-center gap-2 bg-[#F6BCB2] rounded-full text-black px-4 py-1.5">
                        <Calendar size={16} className="text-black" />
                        <span className="text-sm">{formatDate(podcast.createdAt)}</span>
                    </div>
                </div>
