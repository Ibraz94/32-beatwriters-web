...
export default function NuggetsPage() {
  ...
  // Handle helpful marking
  const handleMarkHelpful = async (id: string, isHelpful: boolean) => {
    try {
      await markHelpful({ id, isHelpful }).unwrap()
    } catch (error) {
      console.error('Failed to mark nugget as helpful:', error)
    }
  }
        ...
        <>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayNuggets.map((nugget, index) => (
              <div key={`${nugget.id}-${index}`} className="rounded-xl border shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                {/* Nugget Image */}
                <div className='p-2'>
                <div className="relative w-full h-58 ">
                  {nugget.images && nugget.images.length > 0 ? (
                    <Image
                      src={getImageUrl(nugget.images[0]) || ''}
                      alt={nugget.title}
                      priority
                      fill
                ...
                {/* Nugget Content */}
                <div className="p-6">
                  <h3 className="text-xl font-bold mb-3 line-clamp-2 hover:text-red-600 transition-colors">
                    {nugget.title}
                  </h3>
